...
class MAE(nn.Module):
    def __init__(self, image_channel, image_size, patch_size, enc_dim, dec_dim, encoder, decoder, mask_ratio = 0.75) -> None:
        ...
        # torch.nn.Transformer have the batch dimension in the middle: [T(token), B(batch), D(feature)]
        ...
        self.out_proj = nn.Sequential(
            nn.Linear(dec_dim, self.patch_dim) if dec_dim != self.patch_dim else nn.Identity(),
            nn.Tanh()
        )
    
    def forward(self, img, viz=False, transfer=False):
        ...
        if viz:
            ...
            img_recon = rearrange(img_recon, '(h w) b (s1 s2 c) -> b c (h s1) (w s2)', h=img.shape[2]//self.patch_size, s1=self.patch_size, s2=self.patch_size)
