...
def use_subset(dataset, subset_size):
    ...
    try:
        ...
        print('Missing attribute in dataset')
    return dataset
...
def get_optimizer(model, name, **optimizer_kwargs):
    skip = {'classtoken', 'pos'}
    decay, no_decay = [], []
    ...
    if name == 'AdamW':
        optimizer = torch.optim.AdamW([{'params': no_decay, 'weight_decay': 0}, {'params': decay, 'weight_decay': 5e-2}], lr=0)
    else:
